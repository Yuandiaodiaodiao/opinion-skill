...
name: opinion
description: >
  Opinion prediction market trading and data query skill. Use this skill whenever the user wants to
  interact with Opinion prediction markets — searching markets, checking prices, viewing orderbooks,
  tracking positions, placing buy/sell orders, or managing orders. Trigger this skill when the user
  mentions Opinion markets, prediction markets on Opinion platform, trading on Opinion, market prices,
  orderbook data, or anything related to Opinion CLOB trading. Also use when the user asks about
  crypto prediction market positions, wants to buy/sell outcome tokens, or needs market analytics
  like top markets by volume. Even if the user just says "check my positions" or "what's the price
  of X market" in the context of prediction markets, this skill should activate.
---
...
```bash
ls /root/opinionskills/scripts/config.ts 2>/dev/null && echo "scripts ready" || echo "scripts missing"
```
